...
function injectBillEditor() {
  ...
  document.body.insertAdjacentHTML('beforeend', `
    <dialog id="billEditorDialog" class="bill-editor-dialog">
      <form method="dialog" class="bill-editor-card">
        ...
        <div class="participant-editor"><strong>Participantes</strong><div class="split-note">Puedes poner porcentaje o monto. Si pones porcentaje, se recalcula el monto.</div><div id="editBillParticipants"></div><button class="tiny-button" id="editSplitEven" type="button">Repartir igual</button></div>
  ...
  document.querySelector('#editSplitEven').addEventListener('click', () => splitEditorEven());
...
function splitEditorEven(force = true) {
  ...
  if (!total || !checked.length) return;
  ...
  checked.forEach((checkbox, index) => {
    ...
    if (force || Number(share.value || 0) === 0) share.value = amount;
    pct.value = Math.round(Number(share.value || 0) * 10000 / total) / 100;
  });
}
