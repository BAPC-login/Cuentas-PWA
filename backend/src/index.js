...
async function requestCode(request, env) {
  ...
  if (!user || user.status === 'revoked') {
    return json({ error: 'user_not_allowed', message: 'Este correo no tiene acceso o fue revocado.' }, env, 403);
  }
  ...
  await env.DB.prepare('INSERT INTO login_codes (id, email, code_digest, expires_at) VALUES (?, ?, ?, ?)')
    .bind(id, normalizedEmail, await digest(code), expiresAt)
    .run();

  const delivery = await deliverLoginCode(env, normalizedEmail, code);
  if (delivery.ok) {
    return json({ ok: true, email_sent: true, provider: delivery.provider, expires_at: expiresAt }, env);
  }

  return json({
    ok: true,
    email_sent: false,
    provider: delivery.provider || 'demo',
    warning: delivery.message || 'No se pudo enviar el correo. Se muestra codigo demo.',
    demo_code: code,
...
async function deliverLoginCode(env, to, code) {
  ...
  if (env.RESEND_API_KEY) {
    return sendViaResend(env, to, code);
  }
...
async function sendViaAppsScriptRelay(env, to, code) {
  try {
    ...
    try {
      ...
      if (data.ok) return { ok: true, provider: 'apps_script' };
      return { ok: false, provider: 'apps_script', message: data.error || 'Apps Script rechazo el envio.' };
    } catch {
      return { ok: true, provider: 'apps_script' };
    }
  } catch {
    return { ok: false, provider: 'apps_script', message: 'No hubo conexion con Apps Script.' };
  }
}
...
async function sendViaResend(env, to, code) {
  try {
    ...
    const html = `<p>Tu codigo de acceso a Cuentas Hogar es:</p><h1>${code}</h1><p>Este codigo vence pronto.</p>`;
    ...
    return { ok: false, provider: 'resend', message: normalizeProviderError(detail) };
  } catch {
    return { ok: false, provider: 'resend', message: 'No hubo conexion con Resend.' };
  }
}

function normalizeProviderError(detail) {
  if (!detail) return 'Proveedor rechazo el envio.';
  try {
    const parsed = JSON.parse(detail);
    return parsed.message || parsed.error || detail.slice(0, 220);
  } catch {
...
async function verifyCode(request, env) {
  ...
  ).bind(normalizedEmail, await digest(String(code)), new Date().toISOString()).first();
  ...
  if (!user.name && displayName.length < 2) return json({ error: 'profile_required', message: 'Completa tu nombre para activar el usuario.' }, env, 409);

  if (!user.name && displayName) {
    await env.DB.prepare('UPDATE users SET name = ?, status = ?, activated_at = ? WHERE id = ?')
      .bind(displayName, 'active', new Date().toISOString(), user.id).run();
  }
...
async function updateMyProfile(request, env) {
  ...
  if (displayName.length < 2) return json({ error: 'invalid_name', message: 'El nombre debe tener al menos 2 caracteres.' }, env, 400);
  await env.DB.prepare('UPDATE users SET name = ?, status = ?, activated_at = COALESCE(activated_at, ?) WHERE id = ?')
    .bind(displayName, 'active', new Date().toISOString(), auth.user.id).run();
  return json({ ok: true, user: publicUser(await getUserById(env, auth.user.id)) }, env);
}

async function logout(request, env) {
  const auth = await requireSession(request, env);
  if (!auth.error) await env.DB.prepare('UPDATE sessions SET revoked_at = ? WHERE id = ?').bind(new Date().toISOString(), auth.session.id).run();
...
async function createUser(request, env) {
  ...
  const normalizedEmail = normalizeEmail(email);
  if (!isEmail(normalizedEmail)) return json({ error: 'invalid_email', message: 'Ingresa un correo valido.' }, env, 400);
  if (await getUserByEmail(env, normalizedEmail)) return json({ error: 'email_already_exists', message: 'Ese usuario ya existe.' }, env, 409);
  const id = crypto.randomUUID();
  ...
  await env.DB.prepare('INSERT INTO users (id, email, name, role, status, activated_at) VALUES (?, ?, ?, ?, ?, ?)')
    .bind(id, normalizedEmail, displayName || null, safeRole, displayName ? 'active' : 'pending', displayName ? new Date().toISOString() : null).run();
...
async function setUserStatus(request, env, status) {
  ...
  if (id === auth.user.id) return json({ error: 'cannot_modify_owner_session', message: 'No puedes revocar tu propio owner.' }, env, 400);
  const user = await getUserById(env, id);
  if (!user) return json({ error: 'user_not_found', message: 'Usuario no encontrado.' }, env, 404);
  if (status === 'revoked') {
...
async function ensureOwner(env) {
  ...
  const id = crypto.randomUUID();
  await env.DB.prepare('INSERT INTO users (id, email, name, role, status, activated_at) VALUES (?, ?, ?, ?, ?, ?)')
    .bind(id, ownerEmail, 'Benjamin', 'owner', 'active', new Date().toISOString()).run();
...
async function createSession(env, userId, userAgent) {
  ...
  await env.DB.prepare('INSERT INTO sessions (id, user_id, session_digest, last_seen_at, user_agent) VALUES (?, ?, ?, ?, ?)')
    .bind(id, userId, await digest(raw), new Date().toISOString(), userAgent).run();
...
async function requireOwner(request, env) {
  ...
  if (auth.user.role !== 'owner') return { error: 'owner_required', message: 'Solo el owner puede hacer esto.', status: 403 };
  return auth;
}
...
async function requireSession(request, env) {
  ...
  if (!session) return { error: 'invalid_session', message: 'Sesion invalida o cerrada.', status: 401 };
...
function cors(response, env) { const headers = new Headers(response?.headers || {}); headers.set('access-control-allow-origin', env.APP_ORIGIN || '*'); headers.set('access-control-allow-methods', 'GET,POST,PATCH,OPTIONS'); headers.set('access-control-allow-headers', 'content-type, authorization'); return new Response(response?.body || null, { status: response?.status || 204, headers }); }
