...
async function updateMyProfile(request, env) { const auth = await requireSession(request, env); if (auth.error) return json({ error: auth.error, message: auth.message }, env, auth.status); const { name } = await readJson(request); const displayName = String(name || '').trim(); if (displayName.length < 2) return json({ error: 'invalid_name', message: 'El nombre debe tener al menos 2 caracteres.' }, env, 400); await env.DB.prepare('UPDATE users SET name = ?, status = ?, activated_at = COALESCE(activated_at, ?) WHERE id = ?').bind(displayName, 'active', new Date().toISOString(), auth.user.id).run(); return json({ ok: true, user: publicUser(await getUserById(env, auth.user.id)) }, env); }
...
async function listUsers(request, env) { const auth = await requireOwner(request, env); if (auth.error) return json({ error: auth.error, message: auth.message }, env, auth.status); const { results } = await env.DB.prepare('SELECT id, email, name, role, status, created_at, activated_at, revoked_at FROM users ORDER BY created_at DESC').all(); return json({ users: results }, env); }
...
function publicUser(user) { if (!user) return null; const { id, email, name, role, status, created_at, activated_at, revoked_at } = user; return { id, email, name, role, status, created_at, activated_at, revoked_at }; }
